...
export default function AddressCash() {
    let {paymentCashOrder,setNumberCart}=useContext(CartContext);
    const phoneRegExp =/(002|\+2)?01[1205][0-9]{8}$/;
    let navigate = useNavigate();

    async  function handleAddressCash(values){
    let response =await paymentCashOrder(values);

    if(response?.data.status==='success'){
        toast.success("The order has been ordered successfully",{
          duration: 4000,
          position: 'top-center'})
          setNumberCart(0)
      }
      ...
      let formik=useFormik({
        initialValues: {
                ...
                phone: '',
                city: '',
      },validationSchema,onSubmit:handleAddressCash
    })
      return (<>
    ...
    <form onSubmit={formik.handleSubmit} >
      ...
      <button type='submit' className='btn bg-main text-white mt-3'>Cash on delivery</button>
